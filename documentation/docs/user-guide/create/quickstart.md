...
## Step 4: Deploy (Production Template Only)
...
## Step 6: Programmatic Invocation

Use the AWS SDK `InvokeAgentRuntime` API.
...
* STM (short-term memory) is available immediately
* LTM (long-term memory) can take several minutes to create
...
* Third-party providers use:
  ...
  * `.env` fallback in local dev (`LOCAL_DEV=1`)
