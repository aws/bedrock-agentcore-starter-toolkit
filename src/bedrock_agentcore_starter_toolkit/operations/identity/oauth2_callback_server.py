...
class BedrockAgentCoreIdentity3loCallback(Starlette):
    ...
    def _handle_3lo_callback(self, request: Request) -> JSONResponse:
        ...
        console.print(f'Handling 3LO callback for session_id=[{session_id}] | user_id=[{user_id}]')
        ...
        identity_client.complete_resource_token_auth(session_uri=session_id, user_identifier=UserIdIdentifier(user_id=user_id))
