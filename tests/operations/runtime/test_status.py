...
from bedrock_agentcore_starter_toolkit.operations.runtime.status import get_enhanced_agent_info, get_status
...
class TestGetEnhancedAgentInfo:
    """Test get_enhanced_agent_info functionality."""

    def test_get_enhanced_agent_info_with_source_path(self, tmp_path):
        """Test enhanced info with accessible source path."""
        config_path = tmp_path / ".bedrock_agentcore.yaml"
        ...
        result = get_enhanced_agent_info("test-agent", config_path)

        assert result["agent_name"] == "test-agent"
        assert result["entrypoint"] == "agent.py"
        assert result["source_path"] == str(source_dir)
        assert result["source_path_accessible"] is True
        assert result["entrypoint_exists"] is True
        assert result["deployment_ready"] is True

    def test_get_enhanced_agent_info_without_source_path(self, tmp_path):
        """Test enhanced info for legacy agent without source path."""
        config_path = tmp_path / ".bedrock_agentcore.yaml"
        ...
        project_config = BedrockAgentCoreConfigSchema(
            default_agent="legacy-agent", agents={"legacy-agent": agent_config}
        )
        save_config(project_config, config_path)

        result = get_enhanced_agent_info("legacy-agent", config_path)

        assert result["agent_name"] == "legacy-agent"
        assert result["entrypoint"] == "agent.py"
        assert result["source_path"] is None
        assert result["source_path_accessible"] is False
        assert result["entrypoint_exists"] is False
        assert result["deployment_ready"] is True  # Legacy mode, no source_path check

    def test_get_enhanced_agent_info_with_missing_entrypoint(self, tmp_path):
        """Test enhanced info when entrypoint doesn't exist in source."""
        config_path = tmp_path / ".bedrock_agentcore.yaml"
        source_dir = tmp_path / "source"
        source_dir.mkdir()
        # Create source directory but not the entrypoint file

        agent_config = BedrockAgentCoreAgentSchema(
            name="test-agent",
            entrypoint="missing_agent.py",
            source_path=str(source_dir),
            aws=AWSConfig(
                region="us-west-2",
                account="123456789012",
    ...
    def test_get_enhanced_agent_info_with_build_artifacts(self, tmp_path):
        """Test enhanced info with build artifacts present."""
        from bedrock_agentcore_starter_toolkit.utils.runtime.artifacts import create_build_artifact_organization
        ...
        # Create build artifacts directory using the actual function
        with patch("bedrock_agentcore_starter_toolkit.utils.runtime.artifacts.Path.cwd", return_value=tmp_path):
            create_build_artifact_organization("test-agent")

        with patch("bedrock_agentcore_starter_toolkit.operations.runtime.status.Path.cwd", return_value=tmp_path):
            result = get_enhanced_agent_info("test-agent")

        assert result["build_artifacts_present"] is True
        assert "build_artifacts" in result
        assert result["build_artifacts"]["organized"] is True


class TestStatusOperationMemory:
    """Additional memory-related status tests."""

    def test_status_with_memory_stm_provisioning(self, mock_boto3_clients, tmp_path):
        """Test status for agent with STM-only memory provisioning (covers line 87)."""
        config_path = tmp_path / ".bedrock_agentcore.yaml"
        agent_config = BedrockAgentCoreAgentSchema(
            name="test-agent",
            entrypoint="test.py",
        ...
        with patch(
            ...
            mock_memory_manager = Mock()
            mock_memory_manager.get_memory_status.return_value = "CREATING"
            mock_memory_manager.get_memory.return_value = {"id": "mem-12345"}
            mock_memory_manager.get_memory_strategies.return_value = []
            mock_memory_manager_class.return_value = mock_memory_manager
            ...
            assert result.config.memory_enabled is False
            assert result.config.memory_type == "STM (provisioning...)"
