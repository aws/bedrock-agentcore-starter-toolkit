...
def test_list_actors():
    ...
    with patch("boto3.client"):
        ...
        result, next_token = manager.list_actors("mem-123")
...
def test_list_actors_with_pagination():
    ...
    with patch("boto3.client"):
        ...
        result, next_token = manager.list_actors("mem-123", max_results=1)

        assert len(result) == 1
        assert next_token == "token1"
...
def test_list_sessions():
    ...
    with patch("boto3.client"):
        ...
        result, next_token = manager.list_sessions("mem-123", "actor-1")
...
def test_list_sessions_with_pagination():
    ...
    with patch("boto3.client"):
        ...
        result, next_token = manager.list_sessions("mem-123", "actor-1", max_results=1, next_token="prev_token")

        assert len(result) == 1
        assert next_token == "token1"
        mock_data_plane_client.list_sessions.assert_called_once_with(
            memoryId="mem-123", actorId="actor-1", maxResults=1, nextToken="prev_token"
        )
...
def test_list_actors_with_next_token():
    ...
    with patch("boto3.client"):
        ...
        result, next_token = manager.list_actors("mem-123", next_token="prev_token")

        assert len(result) == 1
        assert next_token is None
        mock_data_plane_client.list_actors.assert_called_once_with(memoryId="mem-123", nextToken="prev_token")
...
def test_list_events():
    ...
    with patch("boto3.client"):
        ...
        result, next_token = manager.list_events("mem-123", "actor-1", "session-1")
...
def test_list_events_paginated():
    """Test list_events with pagination token."""
    with patch("boto3.client"):
        manager = MemoryManager(region_name="us-east-1")
        mock_data_plane_client = MagicMock()
        manager._data_plane_client = mock_data_plane_client
        ...
        result, next_token = manager.list_events(
            "mem-123", "actor-1", "session-1", max_results=50, next_token="token-1"
        )

        assert len(result) == 1
        assert next_token == "token-2"
        mock_data_plane_client.list_events.assert_called_once_with(
            memoryId="mem-123", actorId="actor-1", sessionId="session-1", maxResults=50, nextToken="token-1"
        )
...
def test_list_records():
    ...
    with patch("boto3.client"):
        ...
        result, next_token = manager.list_records("mem-123", "/users/alice/facts/")
...
def test_list_records_paginated():
    """Test list_records with pagination token."""
    with patch("boto3.client"):
        manager = MemoryManager(region_name="us-east-1")
        mock_data_plane_client = MagicMock()
        manager._data_plane_client = mock_data_plane_client
        ...
        mock_data_plane_client.list_memory_records.return_value = {
            "memoryRecordSummaries": [{"recordId": "rec-3"}],
            "nextToken": "token-2",
        }

        result, next_token = manager.list_records("mem-123", "/facts", max_results=50, next_token="token-1")

        assert len(result) == 1
        assert next_token == "token-2"
        mock_data_plane_client.list_memory_records.assert_called_once_with(
            memoryId="mem-123", namespace="/facts", maxResults=50, nextToken="token-1"
        )
