...
class TestBedrockAgentCoreRuntime:
    ...
    def test_get_agent_card(self, mock_boto3_clients):
        ...
        # Mock the SigV4Auth process without capturing the mock
        with patch("botocore.auth.SigV4Auth.add_auth"), patch("requests.get", return_value=mock_response) as mock_get:
            ...
            url = call_args[0][0]
            expected_base_url = f"{client.dp_endpoint}/runtimes/"
            expected_path = "arn%3Aaws%3Abedrock_agentcore%3Aus-west-2%3A123456789012"
            expected_suffix = "%3Aagent-runtime%2Ftest-id/invocations/.well-known/agent-card.json"
            assert url == expected_base_url + expected_path + expected_suffix

            # Verify timeout was provided (fixing the security issue)
            assert call_args[1]["timeout"] == 30

            # Verify result
            assert agent_card["name"] == "TestAgent"
            assert agent_card["description"] == "Test agent description"
            assert len(agent_card["skills"]) == 1

    def test_get_agent_card_with_custom_qualifier(self, mock_boto3_clients):
        """Test getting an agent card with custom qualifier."""
        client = BedrockAgentCoreClient("us-west-2")
        ...
        with patch("botocore.auth.SigV4Auth.add_auth"), patch("requests.get", return_value=mock_response) as mock_get:
            agent_card = client.get_agent_card(
                agent_arn="arn:aws:bedrock_agentcore:us-west-2:123456789012:agent-runtime/test-id", qualifier="CUSTOM"
            )

            # Verify URL includes qualifier parameter
            call_args = mock_get.call_args
            url = call_args[0][0]
            assert "?qualifier=CUSTOM" in url

            # Verify result
            assert agent_card["name"] == "TestAgent"

    def test_get_agent_card_error(self, mock_boto3_clients):
        """Test error handling for get_agent_card."""
        client = BedrockAgentCoreClient("us-west-2")
...
class TestLocalBedrockAgentCoreClient:
    ...
    def test_http_client_get_agent_card(self):
        """Test getting an agent card with HTTP client using bearer token."""
        client = HttpBedrockAgentCoreClient("us-west-2")
        ...
        with patch("requests.get", return_value=mock_response) as mock_get:
            ...
            # Check URL
            url = call_args[0][0]
            expected_base = f"{client.dp_endpoint}/runtimes/"
            expected_path = "arn%3Aaws%3Abedrock_agentcore%3Aus-west-2%3A123456789012"
            expected_suffix = "%3Aagent-runtime%2Ftest-id/invocations/.well-known/agent-card.json"
            assert url == expected_base + expected_path + expected_suffix
    ...
    def test_http_client_get_agent_card_with_custom_qualifier(self):
        """Test getting an agent card with custom qualifier using HTTP client."""
        client = HttpBedrockAgentCoreClient("us-west-2")
    ...
    def test_http_client_get_agent_card_error(self):
        """Test error handling for HTTP client get_agent_card."""
        client = HttpBedrockAgentCoreClient("us-west-2")

        # Test with HTTP error
        with patch("requests.get", side_effect=requests.exceptions.HTTPError("403 Forbidden")):
            with pytest.raises(requests.exceptions.HTTPError):
                client.get_agent_card(
                    agent_arn="arn:aws:bedrock_agentcore:us-west-2:123456789012:agent-runtime/test-id",
