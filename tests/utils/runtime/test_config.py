...
class TestSchemaValidation:
    ...
    def test_invalid_aws_account_validation(self):
        """Test AWS account ID validation with invalid values."""
        from bedrock_agentcore_starter_toolkit.utils.runtime.schema import AWSConfig

        # Test invalid account ID - not 12 digits
        try:
            AWSConfig(account="12345")
            raise AssertionError("Should have raised ValueError")
        except Exception as e:
            assert "Invalid AWS account ID" in str(e)

        # Test invalid account ID - non-numeric
        try:
            AWSConfig(account="abcd12345678")
            raise AssertionError("Should have raised ValueError")
        except Exception as e:
            assert "Invalid AWS account ID" in str(e)

        # Valid account ID should work
        config = AWSConfig(account="123456789012")
        assert config.account == "123456789012"

    def test_build_artifact_info_is_valid(self):
        """Test BuildArtifactInfo is_valid() method."""
        from bedrock_agentcore_starter_toolkit.utils.runtime.schema import BuildArtifactInfo

        # Not organized = not valid
        info = BuildArtifactInfo(organized=False, base_directory="/some/path")
        assert info.is_valid() is False

        # Organized but no base_directory = not valid
        info2 = BuildArtifactInfo(organized=True, base_directory=None)
        assert info2.is_valid() is False

    def test_load_config_with_validation_error(self, tmp_path):
        """Test load_config with validation error handling."""
        ...
        invalid_config = {"agents": {"test": {"name": "test"}}}  # Missing entrypoint and aws
        ...
        try:
            ...
            assert "field is required" in str(e).lower() or "required" in str(e).lower()

    def test_build_artifact_info_cleanup(self, tmp_path):
        """Test BuildArtifactInfo cleanup() method."""
        from bedrock_agentcore_starter_toolkit.utils.runtime.schema import BuildArtifactInfo

        # Create artifact directory
        artifact_dir = tmp_path / ".bedrock-agentcore" / "test-agent"
        artifact_dir.mkdir(parents=True)
        (artifact_dir / "test.txt").write_text("test")

        # Create BuildArtifactInfo
        info = BuildArtifactInfo(base_directory=str(artifact_dir), organized=True, build_timestamp=None)

        # Cleanup should remove directory
        info.cleanup()
        assert not artifact_dir.exists()
        assert info.organized is False
        assert info.build_timestamp is None

    def test_build_artifact_info_cleanup_no_directory(self):
        """Test BuildArtifactInfo cleanup() with no base_directory."""
        from bedrock_agentcore_starter_toolkit.utils.runtime.schema import BuildArtifactInfo

        info = BuildArtifactInfo(base_directory=None)
        # Should not raise
        info.cleanup()
