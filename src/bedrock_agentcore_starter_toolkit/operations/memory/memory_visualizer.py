...
class MemoryVisualizer:
    ...
    def build_record_detail(self, record: Dict[str, Any], verbose: bool = False) -> Panel:
        ...
        lines.append(f"[dim]Namespace:[/dim]  {record.get('_namespace', 'N/A')}")
    ...
    def _get_actors(self, manager: Any, memory_id: str, actor_id: Optional[str], max_actors: int) -> tuple:
        ...
        all_actors, _ = manager.list_actors(memory_id)
    ...
    def _get_sessions(
        ...
        all_sessions, _ = manager.list_sessions(memory_id, actor_id)
    ...
    def _build_session_subtree(
        ...
        try:
            events, _ = manager.list_events(memory_id, actor_id, sid, max_results=max_events)
    ...
    def _add_strategy_records(
        ...
        for ns_template in strategy.get("namespaces", []):
            ...
            for ns in resolved[: DisplayConfig.MAX_ACTORS]:
                try:
                    records, _ = manager.list_records(memory_id, ns, max_results)
                    if records:
