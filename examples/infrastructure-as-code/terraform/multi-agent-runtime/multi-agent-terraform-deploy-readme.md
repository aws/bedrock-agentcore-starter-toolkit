...
1. **A2A Communication**: Orchestrator delegates to specialist via A2A protocol
...
1. Add `coordinator.tf` for the runtime resource
1. Update `s3.tf`, `ecr.tf`, `iam.tf`, `codebuild.tf`
1. Create `buildspec-coordinator.yml`
1. Update `main.tf` for build sequence
1. Update `outputs.tf` and `variables.tf`
...
1. Add domain-specific tools to agents
...
1. Custom authentication layer (if needed)
