...
from bedrock_agentcore_starter_toolkit.create.features.langgraph.feature import LangGraphFeature
...
class TestLangGraphFeature:
    """Tests for LangGraphFeature class."""

    def test_feature_dir_name(self):
        """Test that feature_dir_name is set correctly."""
        assert LangGraphFeature.feature_dir_name == SDKProvider.LANG_GRAPH

    def test_monorepo_dependencies(self, tmp_path):
        """Test monorepo mode dependencies."""
        ctx = create_context(tmp_path, SDKProvider.LANG_GRAPH, ModelProvider.Bedrock, TemplateDirSelection.MONOREPO)
        feature = LangGraphFeature()
        feature.before_apply(ctx)
    ...
    def test_runtime_only_bedrock_dependencies(self, tmp_path):
        """Test runtime_only mode with Bedrock dependencies."""
        ctx = create_context(tmp_path, SDKProvider.LANG_GRAPH, ModelProvider.Bedrock, TemplateDirSelection.RUNTIME_ONLY)
        feature = LangGraphFeature()
        feature.before_apply(ctx)
    ...
    def test_runtime_only_openai_dependencies(self, tmp_path):
        """Test runtime_only mode with OpenAI dependencies."""
        ctx = create_context(tmp_path, SDKProvider.LANG_GRAPH, ModelProvider.OpenAI, TemplateDirSelection.RUNTIME_ONLY)
        feature = LangGraphFeature()
        feature.before_apply(ctx)
    ...
    def test_runtime_only_anthropic_dependencies(self, tmp_path):
        """Test runtime_only mode with Anthropic dependencies."""
        ctx = create_context(
            tmp_path, SDKProvider.LANG_GRAPH, ModelProvider.Anthropic, TemplateDirSelection.RUNTIME_ONLY
        )
        feature = LangGraphFeature()
        feature.before_apply(ctx)
    ...
    def test_runtime_only_gemini_dependencies(self, tmp_path):
        """Test runtime_only mode with Gemini dependencies."""
        ctx = create_context(tmp_path, SDKProvider.LANG_GRAPH, ModelProvider.Gemini, TemplateDirSelection.RUNTIME_ONLY)
        feature = LangGraphFeature()
        feature.before_apply(ctx)
