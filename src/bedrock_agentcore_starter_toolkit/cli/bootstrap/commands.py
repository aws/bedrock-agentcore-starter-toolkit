"""CLI commands for bootstrapping Bedrock AgentCore projects with IAC and SDK scaffolding."""
...
bootstrap_app = typer.Typer(
    name="bootstrap", help="bootstrap an agent core project", invoke_without_command=True, no_args_is_help=False
)
...
def bootstrap(
    ...
    project_name: str = typer.Option(
        None, "--project-name", "-p", prompt="Project name (alphanumeric)", help="Project name to bootstrap"
    ),
    iac: BootstrapIACProvider = iac_option,
    sdk: BootstrapSDKProvider = sdk_option,
):
    """Generate a new Bedrock AgentCore project with infrastructure and SDK scaffolding from configuration."""
    ...
    if Path(project_name).exists():
        raise typer.BadParameter(
            f"A directory already exists with name {project_name}! Either delete that directory or choose a "
            f"new project name."
        )
    ...
    if not configure_yaml.exists():
        _handle_warn(
            "No .bedrock_agentcore.yaml file detected, using bootstrap configuration defaults. To specify "
            "project configuration, first run agentcore configure."
        )
    ...
    if configure_yaml.exists():
        ...
        if len(configure_schema.agents.keys()) > 1:
            _handle_error(
                message="agentcore bootstrap does not currently support multi agent configurations. Try again with a "
                "single agent configured. Exiting."
            )
        ...
        if agent_config.deployment_type != "container":
            _handle_error(
                message="agentcore bootstrap does not currently support direct code deployment. "
                "Run configure again with deployment_type: container"
            )
