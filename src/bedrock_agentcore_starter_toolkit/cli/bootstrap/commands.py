...
def bootstrap(
    ...
    """CLI Implementation for Bootstrap Command."""
    ...
    if configure_yaml.exists():
        ...
        if len(configure_schema.agents.keys()) > 1:
            _handle_error(
                message="agentcore bootstrap does not currently support multi agent configurations. "
    ...
    if not configure_yaml.exists():
        _handle_warn(
            "No .bedrock_agentcore.yaml file detected, using bootstrap configuration defaults. "
