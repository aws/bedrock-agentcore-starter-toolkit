...
1. Creating Cognito User Pool and test user
1. Creating AgentCore Runtime with MCP protocol
...
1. Cognito issues JWT access token
1. Client includes JWT token in MCP request headers
1. Runtime validates token using Cognito's OIDC discovery endpoint
1. Authorized requests processed by MCP server
...
1. Python dependency conflicts in requirements.txt
...
1. Ensure MCP protocol is properly configured
...
1. Ensure allowed_clients matches the client ID
...
1. Check that MCP protocol is configured correctly
1. Ensure the runtime is in ACTIVE state
...
- [Basic Runtime](https://aws.github.io/https:/raw.githubusercontent.com/awslabs/amazon-bedrock-agentcore-samples/refs/heads/main/04-infrastructure-as-code/terraform/basic-runtime/index.md) - Simpler deployment without MCP protocol
