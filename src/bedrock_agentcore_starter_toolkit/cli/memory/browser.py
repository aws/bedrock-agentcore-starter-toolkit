...
class MemoryBrowser:
    ...
    def _clear(self) -> None:
        """Clear the terminal."""
        os.system("cls" if os.name == "nt" else "clear")  # nosec B605 - hardcoded clear command
    ...
    def _render_memory_view(self) -> None:
        ...
        self.console.print()
        nav = Text()
        nav.append("[a] ", style="bold cyan")
        nav.append("Actors (STM)\n")
        nav.append("[n] ", style="bold cyan")
        nav.append("Namespaces (LTM)")
    ...
    def _render_record_detail(self) -> None:
        ...
        if self.current.record_index is not None and self.current.record_index < len(self.data.records):
            ...
            record["_namespace"] = self.current.namespace
            panel = self.visualizer.build_record_detail(record, self.verbose)
            self.console.print(panel)
    ...
    def _load_actors(self, load_more: bool = False) -> None:
        ...
        actors, self.actors_next_token = self.manager.list_actors(
            self.current.memory_id,
    ...
    def _load_sessions(self, load_more: bool = False) -> None:
        ...
        sessions, self.sessions_next_token = self.manager.list_sessions(
            self.current.memory_id,
    ...
    def _load_events(self, load_more: bool = False) -> None:
        ...
        events, self.events_next_token = self.manager.list_events(
            self.current.memory_id,
            self.current.actor_id,
        ...
        self.data.events.sort(key=lambda e: e.get("eventTimestamp", ""))
        self.items = self.data.events
    ...
    def _load_records(self, load_more: bool = False) -> None:
        ...
        records, self.records_next_token = self.manager.list_records(
            self.current.memory_id,
