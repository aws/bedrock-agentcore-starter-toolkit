...
class Observability:
    ...
    def __init__(
        ...
        # Use shared helper - handles config loading, region auto-detection, and client creation
        self.client = _create_observability_client(
            agent=agent_name,
            agent_id=agent_id,
            region=region,
    ...
    def list(
        ...
        if not session_id:
            ...
            session_id = self.client.get_latest_session_id(start_time_ms, end_time_ms)
            if not session_id:
        ...
        spans = self.client.query_spans_by_session(session_id, start_time_ms, end_time_ms)
        ...
        runtime_logs = self.client.query_runtime_logs_by_traces(trace_ids, start_time_ms, end_time_ms)
    ...
    def show(
        ...
        # Show specific trace
        if trace_id:
            _show_trace_view(self.client, trace_id, start_time_ms, end_time_ms, verbose, output)
            # Return TraceData for programmatic use
            spans = self.client.query_spans_by_trace(trace_id, start_time_ms, end_time_ms)
            trace_data = TraceData(spans=spans)
            trace_data.group_spans_by_trace()
            runtime_logs = self.client.query_runtime_logs_by_traces([trace_id], start_time_ms, end_time_ms)
            trace_data.runtime_logs = runtime_logs
            return trace_data
        ...
        if all:
            _show_session_view(self.client, session_id, start_time_ms, end_time_ms, verbose, errors, output)
            # Return TraceData for programmatic use
            spans = self.client.query_spans_by_session(session_id, start_time_ms, end_time_ms)
            trace_data = TraceData(session_id=session_id, spans=spans)
            trace_data.group_spans_by_trace()
            if errors:
                trace_data.traces = trace_data.filter_error_traces()
            trace_ids = list(trace_data.traces.keys())
            runtime_logs = self.client.query_runtime_logs_by_traces(trace_ids, start_time_ms, end_time_ms)
            trace_data.runtime_logs = runtime_logs
            return trace_data
        ...
        # Show Nth most recent trace
            ...
            _show_last_trace_from_session(
                self.client, session_id, start_time_ms, end_time_ms, verbose, last, errors, output
            )
            # Return TraceData for programmatic use
            spans = self.client.query_spans_by_session(session_id, start_time_ms, end_time_ms)
            trace_data = TraceData(session_id=session_id, spans=spans)
