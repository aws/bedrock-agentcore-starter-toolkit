...
class TestDestroyBedrockAgentCore:
    ...
    def test_destroy_with_memory_cleanup(self, mock_session, mock_client_class, tmp_path):
        ...
        # Mock memory client
        with patch(
            "bedrock_agentcore_starter_toolkit.operations.runtime.destroy.MemoryControlPlaneClient"
        ) as mock_memory_client_class:
            ...
            mock_iam_client.list_role_policies.return_value = {"PolicyNames": []}
            mock_iam_client.delete_role.return_value = {}
            mock_memory_client.delete_memory.return_value = {}

            result = destroy_bedrock_agentcore(config_path, dry_run=False)

            # Verify memory deletion was called
            mock_memory_client_class.assert_called_once_with(region_name="us-west-2")
            mock_memory_client.delete_memory.assert_called_once_with(memory_id="mem_123456")
